...
import static model.DirectSystem.STRIKER_VARIANT_PARAMS;
import static model.DirectSystem.STRIKER_VARIANT_PARAMS_TO_SHOW;
import static model.DirectSystem.STRIKER_VARIANT_PARAMS_TO_SHOW_WITH_UNITS;
 ...
 */
public class ReverseSystem extends HydroSystem {
   ...
   private boolean calcParamWithData(String identifer, ArrayList<Double> data) {
        switch(identifer) {
            ...
            case "eta0" : {
                ...
                double sigma1 = data.get(3);
                sigma1 = Math.pow(10, sigma1);
                return calcEta0(x3table, x1table, sigma1);
            }
            case "m" : {
                double sigma1 = data.get(3);
                sigma1 = Math.pow(10, sigma1);
            ...
            case "Sa" : {
                ...
                double sigma1 = data.get(3);
                sigma1 = Math.pow(10, sigma1);
                return calcASurface(x1table, sigma1);
            }
            case "Sb" : {
   ...
   private boolean calcPbg0(double sigma0){
        if(getSystemData().isValueSetted("Sa") &&
                ...
                getSystemData().isValueSetted("sigma0") &&
                getSystemData().isValueSetted("n")){
            double Sa = getSystemData().getValueByIdentifer("Sa", true);
            double Sb = getSystemData().getValueByIdentifer("Sb", true);
            double Pn = getSystemData().getValueByIdentifer("Pn", true);
            double n = getSystemData().getValueByIdentifer("n", true);
        
        double Pbg0 = (Sa * Pn) / (Sb * (1+ sigma0) * (1 - n));
    ...
    private boolean calcXbg0(double xbg0Table){
        if(getSystemData().isValueSetted("Sa") &&
           ...
           getSystemData().isValueSetted("Vn")) {
            ...
            double xbg0 = (xbg0Table * Vn * Math.pow(1-eta0, 1/gamma)) / (gamma * Sa);
    ...
    private boolean calcQ0(double sigma1) {
        if(getSystemData().isValueSetted("eta0") && 
            ...
            double gamma = getSystemData().getValueByIdentifer("gamma", true);
            if (Math.abs(m) > 1e-7) {
                double q0 = Sa * Math.sqrt((Math.pow(1-eta0, 1/gamma-1) * pn * Vn) / (gamma * sigma1 * m));
    ...
    private boolean calcPn(double sigma1) {  
        if(getSystemData().isValueSetted("eta0") && 
                getSystemData().isValueSetted("m") && 
            ...
            double Sa = getSystemData().getValueByIdentifer("Sa", true);
            double gamma = getSystemData().getValueByIdentifer("gamma", true);
            if (Math.abs(Sa) > 1e-7 && Math.abs(Vn) > 1e-7) {
                double Pn = (sigma1 * gamma * m * q0 * q0) / (Sa * Sa * Math.pow(1-eta0, 1/gamma-1) * Vn);
    ...
    private boolean calcEta0(double p3table, double x1table, double sigma1) {
        if(getSystemData().isValueSetted("pn") && 
            ...
            double Sa = getSystemData().getValueByIdentifer("Sa", true);
            double gamma = getSystemData().getValueByIdentifer("gamma", true);
            if (Math.abs(Sa) > 1e-7 && Math.abs(pn) > 1e-7 && Math.abs(Vn) > 1e-7) {
                double value = sigma1 * gamma * m * q0 * q0 / pn / Vn / (Sa) / (Sa);
        ...
        if(getSystemData().isValueSetted("gamma") && 
            ...
            if (Math.abs(x1table) > 1e-7 && Math.abs(Vn) > 1e-7) {
                double value = gamma * (Sa) * x1 / x1table / Vn;
    ...
    private boolean calcM(double sigma1) {
        if(getSystemData().isValueSetted("eta0") && 
            ...
            if (Math.abs(sigma1) > 1e-7 && Math.abs(q0) > 1e-7) {
                double m = (Math.pow(1-eta0, 1/gamma-1) * pn * Vn * Sa * Sa) / (gamma * sigma1 * q0 * q0);
    ...
    private boolean calcX1(double x1table) {
        if(getSystemData().isValueSetted("eta0") && 
            ...
            double gamma = getSystemData().getValueByIdentifer("gamma", true);
            if (Math.abs(Sa) > 1e-7 ) {
                double x1 = (x1table * Math.pow(1-eta0, 1/gamma) * Vn) / (gamma * Sa);
    ...
    private boolean calcASurface(double x1table, double sigma1) {
        if(getSystemData().isValueSetted("pn") && 
                getSystemData().isValueSetted("m") && 
                getSystemData().isValueSetted("q0") && 
                getSystemData().isValueSetted("x1") && 
            ...
            double Sa = Math.pow((x1table * sigma1 * m * q0 * q0 * (1 - eta0)) / (x1 * pn), 1/3);
    ...
    private boolean calcBSurface(double sigma0) {
        if(getSystemData().isValueSetted("pn") && 
            ...
            //if (Math.abs(sigma1) > 1e-7 && Math.abs(m) > 1e-7) {
                double Sb = (Sa * pn) / ((1 + sigma0) * (1 - eta0) * Pbg0);
    ...
    private boolean calcVn(double x1table, double sigma1) {
        if(getSystemData().isValueSetted("eta0") && 
            ...
            if (Math.abs(Sa) > 1e-7 && Math.abs(help) > 1e-7 && Math.abs(pn) > 1e-7) {
                double value = sigma1 * gamma * m * q0 * q0 / (Sa) / (Sa) / help / pn;
    ...
    private boolean calcXmax(double xmaxtable) {
        if(getSystemData().isValueSetted("Vn") &&  
            ...
            double gamma = getSystemData().getValueByIdentifer("gamma", true);
            if (Math.abs(Sa) > 1e-7) {
                double value = xmaxtable * Math.pow(1-eta0, 1/gamma) * Vn / (Sa * gamma);
